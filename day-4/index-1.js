...
const checkWinner = (bingoBoard) => {
  let winner = [];
  const winByRow = checkBingoInRow(bingoBoard);
  const winByColumn = checkBingoInColumn(bingoBoard);
...
const playBingo = (bingoObj, drawArr) => {
  ...
  for (let i = 0; i < drawArr.length; i++) {
    ...
    for (const key in bingoObj) {
      ...
      //check for winner
      winner = checkWinner(bingoObj[key]);

      if (winner.length !== 0) {
        const sum = getRemainingSum(winner);
